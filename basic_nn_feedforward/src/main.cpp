...
int main() {
    ...
    const std::string MNIST_data_path = "/home/sekars/Documents/stage/basic_nn_feedforward/mnist/";


    // Log the paths and check files
    std::cout << "Listing files in MNIST data path: " << MNIST_data_path << '\n';
    for (const auto& entry : std::filesystem::directory_iterator(MNIST_data_path)) {
        std::cout << entry.path() << '\n';
    }
